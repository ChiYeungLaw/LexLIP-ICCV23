...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            output = (prediction_scores, _) + outputs[2:]
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.decoder_heads = nn.ModuleList(
            [BertLayer(self.config) for _ in range(2)])
    ...
    def forward_decoder_heads(
        ...
        decoder_cls = self.decoder_cls if self.decoder_cls is not None else self.cls
        dec_logits = decoder_cls(dec_hidden_states[-1])
