...
def CL4MoCo(
    ...
    # negative loss: N
    t2i_neg = torch.einsum('nc,ck->nk', [text_reps, pl_module.image_queue.clone().detach()]) / pl_module.T
    t2i_logits = torch.cat((ti_pos.unsqueeze(-1), t2i_neg), dim=-1) # [n, k+1]
    
    i2t_neg = torch.einsum('nc,ck->nk', [image_reps, pl_module.text_queue.clone().detach()]) / pl_module.T
    i2t_logits = torch.cat((it_pos.unsqueeze(-1), i2t_neg), dim=-1) # [n, k+1]
